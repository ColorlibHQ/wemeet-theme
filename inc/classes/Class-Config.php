...
final class Wemeet {
	...
	private function setup() {
		...
		$enqueu          = new wemeet_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->wemeet_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'wemeet_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'wemeet', WEMEET_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'wemeet_site_logo_205x45', 205, 45, true );

		// About thumb size
		add_image_size( 'wemeet_about_shape_582x510', 582, 510, true );
		add_image_size( 'wemeet_about_shape_236x236', 236, 236, true );
		add_image_size( 'wemeet_about_img_671x747', 671, 747, true );

		// Speaker thumb size
		add_image_size( 'wemeet_speaker_thumb_460x500', 460, 500, true );
		add_image_size( 'wemeet_speaker_small_thumb_90x90', 90, 90, true );

		// Client logo size
		add_image_size( 'wemeet_client_logo_145x70', 145, 70, true );

		// Footer image size
		add_image_size( 'wemeet_footer_ball_250x250', 250, 250, true );

		// Latest post thumbnail size
		add_image_size( 'wemeet_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'wemeet_single_blog_750x375', 750, 375, true );
		add_image_size( 'wemeet_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'wemeet' ),
			'follow-us'    => esc_html__( 'Follow Us', 'wemeet' ),
			'links' 	   => esc_html__( 'Links', 'wemeet' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = WEMEET_DIR_CSS_URI;
		$jsPath  = WEMEET_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->wemeet_version,
				),
				array(
					'handler' => 'wemeet-style',
			...
			'scripts' => array(
				...
				array(
					...
					'file'       => $jsPath . 'jquery.countdown.js',
					'dependency' => array( 'jquery' ),
					'version'    => '1.0',
					'in_footer'  => true,
				),	
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'wemeet' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate wemeet theme customizer
		$wemeet_theme_customizer = new wemeet_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'wemeet_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'wemeet-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'wemeet_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'wemeet-elementor-default-style',  WEMEET_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->wemeet_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'wemeet_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'wemeet-elementor-notice', WEMEET_DIR_JS_URI.'wemeet-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'wemeet-elementor-notice',
				'wemeetElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'wemeet-elementor-notice-nonce' ),
				)
			);
		}
