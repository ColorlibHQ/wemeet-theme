...
function wemeet_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'wemeet' ),
        'next_string'     => esc_html__( '&raquo;', 'wemeet' ),
        'before_output'   => '<div class="wemeet-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'wemeet_pagination_defaults', $defaults )
    );
