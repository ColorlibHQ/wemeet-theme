...
function wemeet_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'wemeet' ) . '</span></li>';
...
// Blog categories
function wemeet_featured_post_cat(){
...
// social media
if ( ! function_exists( 'wemeet_social' ) ) {
	function wemeet_social( $args = array() ) {
		...
		$url = wemeet_opt( 'wemeet_social_url' );
...
// header cart count
function wemeet_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( WEMEET_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'wemeet' ); ?>">
...
function wemeet_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'wemeet_contact7_form_content', 10, 2 );